        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-12 gap-6 items-start" x-data="chatPage(window.__chatProps)" x-init="init()">
                <aside class="col-span-12 lg:col-span-3 space-y-4">
                    <div class="bg-white shadow-md rounded-xl p-4 border border-slate-200">
                        ...
                        <div class="flex items-center gap-3">
                            <form method="POST" action="{{ route('profile.avatar') }}" enctype="multipart/form-data" class="flex items-center gap-3">
                                @csrf
                                @method('PATCH')
                                <label class="h-10 w-10 rounded-full bg-slate-200 flex items-center justify-center overflow-hidden cursor-pointer hover:ring-2 hover:ring-indigo-500 shadow">
                                    @if($authUser->avatar)
                    ...
                    <div class="bg-sky-50 shadow-sm rounded-xl p-4 border border-sky-100">
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="text-sm font-semibold text-slate-700 uppercase tracking-wide">People</h3>
                            <span class="text-xs text-slate-500">{{ $directUsersPaginator->total() }}</span>
                        </div>
                        <div class="space-y-2" id="dm-list">
                            @forelse($directUsers as $dm)
                                <button
                                    type="button"
                                    class="w-full text-left px-3 py-2 rounded-lg border border-transparent transition flex items-center gap-3 shadow-sm bg-white/80 hover:bg-slate-50"
                                ...
                                >
                                    @if($dm->avatar)
                                        <img src="{{ $dm->avatar }}" alt="{{ $dm->name }}" class="h-8 w-8 rounded-full object-cover shadow" />
                                    @else
                ...
                <section class="col-span-12 lg:col-span-6">
                    @if($rooms->isEmpty() && $directUsers->isEmpty())
                        ...
                        <div class="border-t border-slate-200 px-5 py-4 bg-white rounded-b-xl shadow-sm">
                            <form class="space-y-3">
                                <textarea class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 text-slate-800" rows="3" placeholder="Type a message..." disabled></textarea>
                    ...
                    @else
                        <div class="bg-white shadow-sm rounded-xl border border-slate-200">
                            <div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between bg-sky-50">
                                <div>
                            ...
                            <div class="h-[560px] flex flex-col bg-slate-50">
                                            ...
                                            <div class="flex-1">
                                                <div class="flex items-center justify-between">
                                                    ...
                                                    <button
                                                        ...
                                                        x-show="message.user.id === authUser.id || authUser.role === 'admin'"
                                ...
                                <div class="border-t border-slate-200 px-5 py-4 bg-white rounded-b-xl">
                                    <form @submit.prevent="sendMessage" class="space-y-3">
                                        <textarea x-model="draft" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 text-slate-800" rows="3" placeholder="Type a message..." :disabled="sending || !activeTarget"></textarea>
                    ...
                    <div class="bg-white shadow-sm rounded-xl p-4 border border-slate-200">
                        ...
                        <div class="space-y-2" id="room-list">
                            @forelse($rooms as $room)
                                <div class="flex items-center gap-2">
                                    <button
                                        type="button"
                                        class="flex-1 text-left px-3 py-2 rounded-lg border border-slate-200 bg-white hover:bg-indigo-50 transition font-medium text-slate-800 flex items-center gap-3"
                                        @click="selectTarget('room', {{ $room->id }}, {{ json_encode($room->name) }})"
                                    >
                                        @if($room->avatar)
                                            <img src="{{ $room->avatar }}" alt="Room avatar" class="h-8 w-8 rounded-full object-cover shadow" />
                                        @else
                                            <div class="h-8 w-8 rounded-full bg-slate-200 flex items-center justify-center text-xs font-semibold text-slate-700">{{ strtoupper(substr($room->name, 0, 2)) }}</div>
                                        @endif
                                        <span class="truncate">{{ $room->name }}</span>
                                    </button>
                                    <form method="POST" action="{{ route('rooms.leave', $room) }}" class="ml-1">
                                        @csrf
                                        <button type="submit" class="px-2 py-1 rounded-full text-xs bg-slate-200 text-slate-700 hover:bg-red-100 hover:text-red-600 transition" title="Leave room">Leave</button>
                                    </form>
                                </div>
                            @empty
                        ...
                        @if($authUser->role === UserRole::Admin)
                            <div class="mt-4 text-sm text-indigo-700">
                                <a href="{{ route('admin.rooms.index') }}" class="inline-flex items-center gap-1 font-semibold hover:text-indigo-800">Manage rooms & invitations →</a>
                            </div>
                        @endif
                    </div>

                    <div class="bg-sky-50 shadow-sm rounded-xl p-4 border border-sky-100 space-y-3">
                        <div class="flex items-center justify-between mb-1">
                            <h3 class="text-sm font-semibold text-slate-700 uppercase tracking-wide">Room Members</h3>
                            <span class="text-xs text-slate-500">{{ $roomsAll->count() }}</span>
                        </div>
                        @forelse($roomsAll as $room)
                            <div class="bg-white/80 rounded-lg border border-sky-100 p-3 space-y-2">
                                <div class="flex items-center justify-between">
                                    <p class="text-sm font-semibold text-slate-800">{{ $room->name }}</p>
                                    <span class="text-xs text-slate-500">{{ $room->users_count ?? $room->users->count() }} users</span>
                                </div>
                                <div class="space-y-1 divide-y divide-sky-100">
                                    @if($room->users->isEmpty())
                                        <div class="py-1 text-sm text-slate-400">No users in this room.</div>
                                    @else
                                        @foreach($room->users as $member)
                                            <div class="flex items-center justify-between py-1">
                                                <span class="text-sm text-slate-700">{{ $member->name ?? '[no name]' }}</span>
                                                @if(auth()->user()->can('kick', $room) && $member->id !== auth()->id())
                                                    <form method="POST" action="{{ route('rooms.kick', [$room, $member]) }}">
                                                        @csrf
                                                        <button type="submit" class="text-xs text-red-600 hover:text-red-700 font-semibold">Kick</button>
                                                    </form>
                                                @endif
                                            </div>
                                        @endforeach
                        ...
                        @empty
                            <p class="text-sm text-slate-500">No rooms loaded.</p>
                        @endforelse
                    </div>
